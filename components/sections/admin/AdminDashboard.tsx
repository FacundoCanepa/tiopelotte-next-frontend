...
export default function AdminDashboard() {
  ...
  // Filtros para la tabla de pedidos
  ...
  // Filtros para el resumen y métricas generales
  ...
  const porMercadoPago = resumenFiltrado.filter((p) =>
    p.tipoPago?.toLowerCase() === "mercado pago"
  ).length;
  const porEfectivo = resumenFiltrado.filter((p) => p.tipoPago === "efectivo").length;
  const porAElegir = resumenFiltrado.filter((p) =>
    p.tipoPago?.toLowerCase().includes("elegís")
  ).length;
